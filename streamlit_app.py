...
# 1. URL DE L'APPS SCRIPT (La teua Versió 5)
...
traduccions = {
    "Valencià": {
        ...
        "botó_volcar": "💾 REGISTRAR VOLCAT", "recomenat": "Pes net suggerit:", 
        "hui": "📋 Registre de hui (🗑️ per esborrar):", "exit": "✅ Registrat!"
    },
    "Castellano": {
        ...
        "botó_volcar": "💾 REGISTRAR VOLCADO", "recomenat": "Peso neto sugerido:", 
        "hui": "📋 Registro de hoy (🗑️ para borrar):", "exit": "✅ ¡Hecho!"
    },
    "Română": {
        ...
        "botó_volcar": "💾 ÎNREGISTREAZĂ DESCĂRCAREA", "recomenat": "Greutate netă sugerată:", 
        "hui": "📋 Înregistrări astăzi (🗑️ șterge):", "exit": "✅ Gata!"
    }
}

# 3. CONFIGURACIÓ I ESTIL CSS
...
# 4. FUNCIÓ LLEGIR DADES (Directa i robusta)
@st.cache_data(ttl=2)
def carregar_dades():
    try:
    ...
    if not df_total.empty:
        ...
        if not dis.empty:
            ...
            with st.form("v"):
                ...
                vr = round(vc * (d_l['Kg_in']/d_l['Cajas_in']), 2) if d_l['Cajas_in']>0 else 0.0
                st.write(f"💡 {t['recomenat']} {vr} Kg")
                v_real = st.number_input("Kg:", value=float(vr))
                if st.form_submit_button(t["botó_volcar"]):
                    dv = {"Fecha": datetime.now().strftime("%d/%m/%Y"), "Finca": d_l['Finca'], "Parcela": "VOLCAT", "Tipo": "Recogido", "Kg": str(v_real).replace('.',','), "Cajas": int(vc), "ID_Lote": f"V-{datetime.now().strftime('%M%S')}", "Ref_Original": d_l['ID_Lote']}
