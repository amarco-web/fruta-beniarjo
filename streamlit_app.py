...
# 2. TRADUCCIONS COMPLETES (CORREGIT: Afegit n_caps_volcar)
traduccions = {
    "Valencià": {
        ...
        "tria_lot": "Selecciona el Lot:", "dispo": "Disponible:", "n_caps_volcar": "Quantes caixes vas a bolcar?",
        "botó_volcar": "🚜 REGISTRAR VOLCAT", "recomenat": "Pes suggerit:", "hui": "📋 Registres recents (🗑️ per esborrar):", "exit": "✅ Fet!"
    },
    "Castellano": {
        ...
        "tria_lot": "Selecciona el Lote:", "dispo": "Disponible:", "n_caps_volcar": "¿Cuántas cajas vas a volcar?",
        "botó_volcar": "🚜 REGISTRAR VOLCADO", "recomenat": "Peso sugerido:", "hui": "📋 Registros recientes (🗑️ para borrar):", "exit": "✅ ¡Hecho!"
    },
    "Română": {
        ...
        "afegeix": "➕ ADAUGĂ CÂNTĂRIRE", "net_total": "Total Lot Acumulat", "botó_guardar": "💾 SALVEAZĂ ÎN SHEET",
        "tria_lot": "Selectați lotul:", "dispo": "Disponibil:", "n_caps_volcar": "Câte lăzi?",
        "botó_volcar": "💾 ÎNREGISTREAZĂ DESCĂRCAREA", "recomenat": "Greutate sugerată:", "hui": "📋 Înregistrări recente (🗑️ șterge):", "exit": "✅ Gata!"
    }
}
...
if opcio == t["menu_entrada"]:
    ...
    v_tc = col1.number_input(t["tara_caixa"], value=0.50, step=0.01)
    v_tp = col2.number_input(t["tara_palet"], value=15.0, step=0.1)
    ...
    c1, c2 = st.columns(2)
    v_nc = c1.number_input(t["n_caps"], value=1, min_value=0)
    v_np = c2.number_input(t["n_palets"], value=1, min_value=0)

    if st.button(t["afegeix"], type="secondary", use_container_width=True):
        t_net = round(p_brut - ((v_nc * v_tc) + (v_np * v_tp)), 2)
        st.session_state.llista_pesades.append({"net": t_net, "c": v_nc})
    ...
    if st.session_state.llista_pesades:
        ...
        if st.button(t["botó_guardar"], type="primary", use_container_width=True):
            # ID ÚNIC (Timestamp + Microsegons) per evitar el Duplicate Key Error
    ...
    if not df_total.empty:
        # Mostrem últims 10 registres amb claus úniques
        for i, r in df_total.tail(10).iloc[::-1].iterrows():
            c1, c2 = st.columns([4, 1])
            data_neta = str(r['Fecha']).split('T')[0] # Neteja de format data Google
            c1.write(f"📅 {data_neta} | {r['Finca']} | **{r['Kg']}kg** ({r['Tipo']})")
            # ÚS DE CLAU ÚNICA COMBINADA (del_{i}_{lote})
...
# --- SECCIÓ 2: VOLCAT ---
    ...
    if not df_total.empty:
        ...
        if not dispo.empty:
            ...
            with st.form("v"):
                v_c = st.number_input(t["n_caps_volcar"], min_value=1, max_value=int(d_l['Cap_N']) if d_l['Cap_N'] > 0 else 100, value=1)
                mitjana = d_l['Kg_in']/d_l['Cajas_in'] if d_l['Cajas_in']>0 else 0
                vr = st.number_input("Kg:", value=float(round(v_c * mitjana, 2)))
                if st.form_submit_button(t["botó_volcar"]):
                    dv = {"Fecha": datetime.now().strftime("%d/%m/%Y"), "Finca": d_l['Finca'], "Parcela": "VOLCAT", "Tipo": "Recogido", "Kg": str(round(vr,2)).replace('.',','), "Cajas": int(v_c), "ID_Lote": f"V-{int(time.time())}", "Ref_Original": d_l['ID_Lote']}
